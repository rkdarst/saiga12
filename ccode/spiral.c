int cycleSpiral(struct SimData *SD, double n) {

  printf("error: Using Spiral without event driven dynamics does NOT WORK(yet)\n");
...
inline int EddSpiral_isMobile(struct SimData *SD, int pos) {
...
#define DOWN(x) (SD->lattsite[(x)] == S12_EMPTYSITE)
  if (((DOWN(N) && DOWN(NE)) || (DOWN(S) && DOWN(SW))) &&
      ((DOWN(E) && DOWN(ES)) || (DOWN(W) && DOWN(WN))) ) {
    return 1;
    }
  return 0;
}
...
void EddSpiral_init(struct SimData *SD) {
  int pos;
  for (pos=0 ; pos<SD->lattSize ; pos++) {
    if (debugedd)
      printf("EddSpiral_init: at pos: %d\n", pos);
    EddSpiral_updateLatPos(SD, pos);
  }
}
...
inline void EddSpiral_updateLatPos(struct SimData *SD, int pos) {
  ...
  int isAllowedMove=0;
  if (debugedd)
    printf("EddSpiral_updateLatPos: at update: pos:%d\n", pos);


  int state = SD->lattsite[pos] != S12_EMPTYSITE;
  if (debugedd)
    printf("EddSpiral_updateLatPos: at update: pos:%d, state=%d\n", pos, state);
  if (SD->flags & S12_FLAG_FROZEN  &&  SD->frozen[pos]) {
    isAllowedMove = 0;
    }
  else if (EddSpiral_isMobile(SD, pos)) {
    isAllowedMove = 1;
  }
...
inline void EddSpiral_updateLatPos2(struct SimData *SD, int pos, struct LList *llist) {
...
inline void EddSpiral_updateLatNeighbors(struct SimData *SD, int pos) {
  int N  = SD->conn[pos*4+0];
  int NE = SD->conn[N  *4+1];
  int E  = SD->conn[pos*4+1];
  int ES = SD->conn[E  *4+2];
  ...
  int WN = SD->conn[W  *4+0];
  if (debugedd)
    printf("EddSpiral_updateLatPos: at update: pos:%d, neigh=%d %d %d %d %d %d %d %d\n", pos,
	   N, NE, E, ES, S, SW, W, WN);
  EddSpiral_updateLatPos(SD, N );
  EddSpiral_updateLatPos(SD, NE);
  EddSpiral_updateLatPos(SD, E );
  EddSpiral_updateLatPos(SD, ES);
  EddSpiral_updateLatPos(SD, S );
  EddSpiral_updateLatPos(SD, SW);
  EddSpiral_updateLatPos(SD, W );
  EddSpiral_updateLatPos(SD, WN);
}


int EddSpiral_consistencyCheck(struct SimData *SD) {
...
int EddSpiral_cycle(struct SimData *SD, double n) {
  ...
  while (time < maxTime) {
    ...
    EddSpiral_updateLatNeighbors(SD, pos);
