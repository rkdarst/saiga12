...
inline int EddSPM_isMobile(struct SimData *SD, int pos) {
  return 1;
}


void EddSPM_init(struct SimData *SD) {
  int pos;
  for (pos=0 ; pos<SD->lattSize ; pos++) {
    if (debugedd)
      printf("EddSPM_init: at pos: %d\n", pos);
    EddSPM_updateLatPos(SD, pos);
  }
}

inline void EddSPM_updateLatPos(struct SimData *SD, int pos) {
  // iterate through all connections, see if each is correctly satisfied...
  // 
  int isAllowedMove=0;
  if (debugedd)
    printf("EddSPM_updateLatPos: at update: pos:%d\n", pos);


  int state = SD->lattsite[pos] != S12_EMPTYSITE;
  if (debugedd)
    printf("EddSPM_updateLatPos: at update: pos:%d, state=%d\n", pos, state);
  if (SD->flags & S12_FLAG_FROZEN  &&  SD->frozen[pos]) {
    isAllowedMove = 0;
    }
  else if (EddSPM_isMobile(SD, pos)) {
    isAllowedMove = 1;
  }
...
inline void EddSPM_updateLatPos2(struct SimData *SD, int pos, struct LList *llist) {
...
inline void EddSPM_updateLatNeighbors(struct SimData *SD, int pos) {
  ...
  if (debugedd)
    printf("EddSPM_updateLatPos: at update: pos:%d, neigh=%d %d %d %d %d %d %d %d\n", pos,
	   N, NE, E, ES, S, SW, W, WN);
  EddSPM_updateLatPos(SD, N );
  EddSPM_updateLatPos(SD, NE);
  EddSPM_updateLatPos(SD, E );
  EddSPM_updateLatPos(SD, ES);
  EddSPM_updateLatPos(SD, S );
  EddSPM_updateLatPos(SD, SW);
  EddSPM_updateLatPos(SD, W );
  EddSPM_updateLatPos(SD, WN);
}


int EddSPM_consistencyCheck(struct SimData *SD) {
